...
- `backend/`：Spring Boot 3 + Java 21 + MyBatis-Plus，提供 REST API
- `frontend/`：Vue 3 + TypeScript + Vite + Element Plus（组合式 API + `<script setup>`）
- `database/`：字典与初始化数据 CSV；`docker-compose.yml`：MySQL / Redis / MinIO
